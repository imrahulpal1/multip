...
// ── DoubtCard ──────────────────────────────────────────────────────────────
function DoubtCard({ q, viewerLanguage, onUpvote, onReply }) {
    ...
    <div className="rounded-xl border border-white/10 bg-slate-900/40 p-4 space-y-3">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="flex-1 min-w-0">
          <p className="font-medium text-white leading-snug">{displayTitle}</p>
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            <span className="text-xs text-slate-400">
              Posted in <span className="text-indigo-300">{sourceLang}</span>
            </span>
            {!isSameLang && (
              <button
              ...
              >
                <Languages size={11} />
                {showOriginal ? `Show ${viewerLanguage}` : `Show original (${sourceLang})`}
              </button>
            )}
            {q.technicalTerms?.length > 0 && q.technicalTerms.map((t) => (
      ...
      <p className="rounded-lg bg-slate-800/60 p-3 text-sm text-slate-200 leading-relaxed whitespace-pre-wrap">
        {displayContent}
      </p>
      ...
      <div className="flex gap-2 pt-1">
        <input
          ...
          className="flex-1 rounded-lg border border-white/10 bg-slate-900/40 px-3 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-400"
        />
...
function ReplyCard({ reply, viewerLanguage }) {
  ...
  return (
    <div className="rounded-lg bg-slate-800/40 p-3 text-sm text-slate-200 space-y-1">
...
export default function PeerLearningPage() {
  ...
  // Set auth token from Clerk email
  useEffect(() => {
  ...
  // Socket.io — real-time updates for ALL users (gracefully degrades if server is down)
  useEffect(() => {
    const socket = io(SOCKET_URL, {
      transports: ['polling', 'websocket'], // polling first — avoids WS error on cold start
    ...
    socket.on('connect_error', () => { /* silent — server may be starting up */ })
  ...
  return (
    <div className="space-y-5">
      <Card title="Peer Learning Hub" subtitle="Ask doubts in your language — everyone sees it in theirs">
        <div className="mb-3 flex justify-end">
          <Button onClick={() => { setError(''); setIsOpen(true) }}>Ask Question</Button>
        </div>
        <div className="space-y-3">
          {forumThreads.length === 0 && (
            <p className="text-center text-sm text-slate-500 py-8">No questions yet — be the first to ask!</p>
          )}
          {forumThreads.map((q) => (
            <DoubtCard
              key={q._id || q.id}
      ...
      {isOpen && (
        <div className="fixed inset-0 z-20 grid place-items-center bg-slate-950/70 px-4">
          <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-slate-900 p-5 space-y-4">
            <h3 className="text-lg font-semibold text-white">Ask a New Question</h3>

            {error && (
              <div className="flex items-center gap-2 rounded-lg bg-red-500/10 border border-red-400/30 px-3 py-2 text-sm text-red-300">
